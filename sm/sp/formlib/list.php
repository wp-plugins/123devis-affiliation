	...
	class sm_formlib_list extends sm_renderable {
		...
		function render($data, $form){
			...
			foreach ($data['options'] as $option){
				//basics
				$s .= "<input type=\"{$this->form_type}\" name=\"{$data['name']}\" id=\"form_{$option['value']}\" value=\"{$option['value']}\" ";
				...
				$s .= "><label class=\"sm_input_label\" for=\"form_{$option['value']}\">{$option['label']}</label><br>\n";
			}
			$s .= "</div>\n";
